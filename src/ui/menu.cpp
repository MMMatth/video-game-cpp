...
Menu::Menu(RenderWindow &window)
    : m_menu(true), m_window(window), m_sound(), m_newGame(false),
      m_soundSettings(5), m_clickOnOff(2) {
...
void Menu::handleEvent(sf::Event &event) {
  if (event.type == sf::Event::MouseButtonPressed && m_menu) {
    ...
    if (isInside(mouseX, mouseY, 37, 35, 65, 63)) {
      if (m_clickOnOff == 2) {
        Error(!menuTexture.loadFromFile(IMG_MENU_OFF),
              "Error loading menuOff texture");
        m_soundSettings.setVolume(0);
        m_sound.setVolume(m_soundSettings.getVolume());
        m_clickOnOff--;
      } else {
        Error(!menuTexture.loadFromFile(IMG_MENU_ON),
              "Error loading menuOn texture");
